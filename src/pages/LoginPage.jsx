...
export default function LoginPage() {
  ...
  async function handlePasswordReset(e) {
    ...
    setSuccess('Password updated! Signing you out — please log in with your new password.')
  ...
  const cardStyle = {
    ...
    background: '#080808', border: '1px solid #1a1a1a', padding: '48px 52px',
  }
  ...
  return (
    <div style={wrap}>
      ...
      {/* ── PASSWORD RESET MODE ── */}
      {mode === 'reset' && (
        <form onSubmit={handlePasswordReset} style={cardStyle}>
          <div style={{ background: 'rgba(67,97,238,0.1)', border: '1px solid rgba(67,97,238,0.3)', color: '#7ca4ff', padding: '12px 16px', fontSize: '12px', marginBottom: '28px', letterSpacing: '0.04em' }}>
            🔒 You clicked a password reset link. Enter your new password below.
          </div>
          {error && (
            <div style={{ background: 'rgba(255,68,68,0.1)', border: '1px solid #ff4444', color: '#ff4444', padding: '12px 16px', fontSize: '12px', marginBottom: '20px' }}>
              {error}
            </div>
          )}
          {success && (
            <div style={{ background: 'rgba(0,204,102,0.1)', border: '1px solid #00cc66', color: '#00cc66', padding: '12px 16px', fontSize: '12px', marginBottom: '20px' }}>
              {success}
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <Field label="NEW PASSWORD" name="password" type="password" value={form.password}
              onChange={e => setForm(p => ({ ...p, password: e.target.value }))}
              placeholder="Min. 8 characters" autoComplete="new-password" />
            <Field label="CONFIRM NEW PASSWORD" name="confirm" type="password" value={form.confirm}
              onChange={e => setForm(p => ({ ...p, confirm: e.target.value }))}
              placeholder="Repeat new password" autoComplete="new-password" />
          </div>
          <button type="submit" disabled={loading} style={{ width: '100%', background: loading ? '#2a2a2a' : '#4361ee', color: loading ? '#666' : '#fff', border: 'none', padding: '16px', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '14px', letterSpacing: '0.12em', fontFamily: 'inherit', marginTop: '32px' }}>
